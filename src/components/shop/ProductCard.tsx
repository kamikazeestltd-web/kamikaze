    ...
    <Link href={`/product/${product.slug}`} className={`group block ${product.soldOut ? 'opacity-50' : ''}`}>
      <div className="relative aspect-[3/4] overflow-hidden bg-[#0a0a0a] mb-4">
        <Image
          ...
          alt={product.name}
          fill
          className={`object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700 opacity-80 group-hover:opacity-100 ${product.soldOut ? 'group-hover:grayscale' : ''}`}
        />
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          {product.soldOut && (
            <span className="text-[9px] tracking-[0.2em] uppercase bg-red-600/80 text-white px-2 py-1 font-black">
              Sold Out
            </span>
          )}
        </div>
        {/* Quick view overlay */}
        <div className={`absolute inset-x-0 bottom-0 bg-black/80 py-3 text-center text-[9px] tracking-[0.3em] uppercase text-white/60 transition-transform duration-300 ${product.soldOut ? '' : 'translate-y-full group-hover:translate-y-0'}`}>
